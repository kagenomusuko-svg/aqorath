...
def _effect(
    ...
    *,
    ...
    snapshot = FiscalMonetaryConfirmationSnapshot(
        fact_type="sale",
        fact_amount=(accounting.fact.amount if fact_amount is None else Decimal(fact_amount)),
        payment_method="cash",
...
def _effects(accounting):
    return (
        _effect(accounting, rule_key="iva.general_rate", rate="0.16", exact_amount="16.0000", rounded_amount="16.00", role="vat_payable", side="credit", source_ref="TEST:IVA-TRASLADADO"),
        _effect(accounting, rule_key="iva.withholding", rate="0.10666666", exact_amount="10.666666", rounded_amount="10.67", role="vat_withheld_receivable", side="debit", source_ref="TEST:IVA-RETENIDO"),
        _effect(accounting, rule_key="isr.withholding", rate="0.0125", exact_amount="1.2500", rounded_amount="1.25", role="isr_withheld_receivable", side="debit", source_ref="TEST:ISR-RETENIDO"),
    )


def _declaration(*, fact_amount="100.00", basis="net_before_fiscal", adjustment_role="cash"):
    ...
    return declare_fiscal_economic_composition(accounting, effects, basis, adjustment_role)
...
def _resolved_multi(monkeypatch):
    ...
    return resolve_fiscalized_proposal_accounts(object(), fiscalized, {
        "cash": "CASH", "sales_revenue": "SALES", "vat_payable": "VAT-P",
        "vat_withheld_receivable": "VAT-W", "isr_withheld_receivable": "ISR-W",
    })
...
def test_composition_accepts_ordered_immutable_effect_tuple_without_breaking_single_effect_contract():
    from aqorath.fiscal_economic_composition import FiscalEconomicCompositionDeclaration, declare_fiscal_economic_composition
    accounting = _accounting()
    effects = _effects(accounting)
    declaration = declare_fiscal_economic_composition(accounting, effects, "net_before_fiscal", "cash")
    ...
    assert tuple(effect.line.account_role for effect in declaration.fiscal_effects) == ("vat_payable", "vat_withheld_receivable", "isr_withheld_receivable")
...
def test_composition_rejects_mutable_empty_malformed_or_cross_fact_effect_collections():
    ...
    for invalid in ([], (), (effects[0], object())):
        with pytest.raises((TypeError, ValueError)):
            declare_fiscal_economic_composition(accounting, invalid, "net_before_fiscal", "cash")
    mismatched = _effect(accounting, rule_key="isr.withholding", rate="0.0125", exact_amount="1.2500", rounded_amount="1.25", role="isr_withheld_receivable", side="debit", source_ref="TEST:MISMATCH", fact_amount="99.00")
    with pytest.raises(ValueError, match="fact_amount|economic|fiscal"):
        declare_fiscal_economic_composition(accounting, (effects[0], effects[1], mismatched), "net_before_fiscal", "cash")
...
def test_net_before_fiscal_composes_iva_and_two_withholdings_as_exact_balanced_five_line_truth():
    ...
    assert tuple((line.account_role, line.side, line.amount) for line in proposal.lines) == (
        ("cash", "debit", Decimal("104.08")), ("sales_revenue", "credit", Decimal("100.00")),
        ("vat_payable", "credit", Decimal("16.00")), ("vat_withheld_receivable", "debit", Decimal("10.67")),
...
def test_gross_including_fiscal_uses_aggregate_effect_direction_and_reaches_same_truth():
    from aqorath.fiscalized_accounting_proposal import compose_fiscal_economic_accounting
    proposal = compose_fiscal_economic_accounting(_declaration(fact_amount="104.08", basis="gross_including_fiscal", adjustment_role="sales_revenue"))
    assert tuple((line.account_role, line.side, line.amount) for line in proposal.lines) == (
        ("cash", "debit", Decimal("104.08")), ("sales_revenue", "credit", Decimal("100.00")),
        ("vat_payable", "credit", Decimal("16.00")), ("vat_withheld_receivable", "debit", Decimal("10.67")),
        ("isr_withheld_receivable", "debit", Decimal("1.25")),
...
def test_confirmation_preserves_all_effect_provenance_in_order_and_matches_trailing_fiscal_lines(monkeypatch):
    ...
    assert tuple(effect.rule_key for effect in effects) == ("iva.general_rate", "iva.withholding", "isr.withholding")
    assert tuple(effect.fiscal_role for effect in effects) == ("vat_payable", "vat_withheld_receivable", "isr_withheld_receivable")
    ...
    assert tuple((line.account_role, line.side, line.amount) for line in trailing) == tuple((effect.fiscal_role, effect.fiscal_side, effect.rounded_fiscal_amount) for effect in effects)
...
def test_multi_effect_posting_instruction_preserves_all_confirmed_positive_lines_and_balance(monkeypatch):
    ...
    instruction = create_fiscalized_posting_instruction(confirmed, "reject_zero_fiscal_line")
    ...
    assert tuple(line.account_role for line in instruction.lines) == ("cash", "sales_revenue", "vat_payable", "vat_withheld_receivable", "isr_withheld_receivable")
...
def test_audit_snapshot_deep_copies_complete_multi_effect_provenance_without_recalculation(monkeypatch):
    ...
    assert all(copied is not source for copied, source in zip(result.provenance.fiscal_effects, snapshot.provenance.fiscal_effects))
...
def test_schema_additively_ensures_ordered_additional_effect_records_without_bumping_frozen_v4(tmp_path):
    ...
    try:
        ...
        conn.commit()
        assert conn.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='fiscalpostingauditeffectrecord'").fetchone() is None
    finally:
        conn.close()
    ...
    conn = sqlite3.connect(str(db))
    try:
        columns = {row[1]: row[2] for row in conn.execute("PRAGMA table_info(fiscalpostingauditeffectrecord)")}
        assert columns
        ...
        for name in ("base", "rate", "exact_fiscal_amount", "rounding_quantizer", "rounded_fiscal_amount"):
...
def test_atomic_persistence_and_read_round_trip_three_effects_exactly_without_recalculation(tmp_path, monkeypatch):
    ...
    from aqorath.fiscalized_account_resolution import resolve_fiscalized_proposal_accounts
    from aqorath.fiscalized_accounting_proposal import compose_fiscal_economic_accounting
    from aqorath.models import Account, FiscalPostingAuditEffectRecord, FiscalPostingAuditRecord, JournalEntry, JournalLine
    ...
    try:
        with Session(engine) as session:
            ...
            declaration = _declaration()
            fiscalized = compose_fiscal_economic_accounting(declaration)
            resolved = resolve_fiscalized_proposal_accounts(session, fiscalized, {"cash": "1102", "sales_revenue": "4201", "vat_payable": "2103", "vat_withheld_receivable": "1103", "isr_withheld_receivable": "1105"})
            ...
            instruction = application.create_fiscalized_posting_instruction(confirmed, "reject_zero_fiscal_line")
        ...
        with Session(engine) as session:
            ...
            additional = session.exec(select(FiscalPostingAuditEffectRecord).order_by(FiscalPostingAuditEffectRecord.position)).all()
            assert len(entries) == 1
            assert len(lines) == 5
            assert len(audits) == 1
            assert len(additional) == 2
            assert [record.position for record in additional] == [1, 2]
            assert [record.rule_key for record in additional] == ["iva.withholding", "isr.withholding"]
            ...
            monkeypatch.setattr(calculation, "calculate_fiscal_rate_amount", forbidden)
            monkeypatch.setattr(rounding, "round_confirmed_fiscal_amount", forbidden)
            loaded = application.load_fiscal_posting_audit_snapshot(session, result["entry_id"])
        expected = confirmation.provenance.fiscal_effects
        actual = loaded.provenance.fiscal_effects
        assert len(actual) == 3
        assert tuple(effect.rule_key for effect in actual) == tuple(effect.rule_key for effect in expected)
