"""Phase 3D.3 — confirmed supplier payment persists exactly through SQLite authority."""

from decimal import Decimal


def test_supplier_payment_confirmed_truth_persists_to_sqlite(tmp_path, monkeypatch):
    ...
    db_path = tmp_path / "supplier-payment-e2e.db"
    ...
    with Session(engine) as session:
        ...
        bank_id = bank.id
        session.add(AccountRoleBinding(role="accounts_payable", account_id=payable_id))
    ...
    with Session(engine) as session:
        ...
        assert set(by_code) == {"2101", "1101"}

        payable_line = by_code["2101"]
        assert payable_line.account_id == payable_id
        assert Decimal(str(payable_line.debit)) == Decimal("800.00")
        assert Decimal(str(payable_line.credit)) == Decimal("0")

        bank_line = by_code["1101"]
        assert bank_line.account_id == bank_id
        assert Decimal(str(bank_line.debit)) == Decimal("0")
        assert Decimal(str(bank_line.credit)) == Decimal("800.00")
