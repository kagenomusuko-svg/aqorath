...
def _period_db(tmp_path):
    ...
    try:
        ...
        conn.executemany(
            "INSERT INTO journalline(id,entry_id,account_code,account_id,debit,credit,description) VALUES(?,?,?,?,?,?,?)",
            (
                (1, 1, "1101", 1, "100", "0", "Apertura banco"),
                (2, 1, "1104", 2, "100", "0", "Apertura inventario"),
                (3, 1, "3101", 3, "0", "200", "Apertura patrimonio"),
            ),
        )
        conn.execute(
        ...
        conn.executemany(
            "INSERT INTO journalline(id,entry_id,account_code,account_id,debit,credit,description) VALUES(?,?,?,?,?,?,?)",
            (
                (4, 2, "1101", 1, "100", "0", "Cobro"),
                (5, 2, "4101", 4, "0", "100", "Ingreso"),
                (6, 2, "5101", 5, "60", "0", "Costo"),
                (7, 2, "1104", 2, "0", "60", "Salida inventario"),
            ),
        )
        conn.commit()
    finally:
...
def _wire_runtime(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(app._storage, "get_db_path", lambda: path)
    monkeypatch.setattr(app._accounting_rules, "load_catalog", _catalog)
...
def test_period_source_does_not_leak_post_range_movements(tmp_path):
    ...
    try:
        ...
        conn.executemany(
            "INSERT INTO journalline(id,entry_id,account_code,account_id,debit,credit,description) VALUES(?,?,?,?,?,?,?)",
            (
                (8, 3, "1101", 1, "999", "0", "later"),
                (9, 3, "4101", 4, "0", "999", "later"),
            ),
        )
        conn.commit()
    finally:
...
def test_first_package_uses_same_canonical_bundle_as_individual_reports(tmp_path, monkeypatch):
    ...
    prepared = app.prepare_financial_period_package(
        object(), from_date=date(2026, 1, 1), to_date=date(2026, 1, 31), format="json",
    )
    assert tuple(request.report_definition_id for request in prepared.requests) == (1301, 1302, 1303)
    package_result = app.build_financial_period_package(object(), prepared)
    individual = tuple(app.build_report_product(object(), request) for request in prepared.requests)
...
def test_journal_and_general_ledger_reconcile_to_same_period_truth(tmp_path, monkeypatch):
    app = _wire_runtime(tmp_path, monkeypatch)
    journal_request = app.prepare_report_request(
        object(), 1304,
        from_date=date(2026, 1, 1), to_date=date(2026, 1, 31), format="json",
    )
    ledger_request = app.prepare_report_request(
        object(), 1305,
        from_date=date(2026, 1, 1), to_date=date(2026, 1, 31), format="json",
    )
    journal = app.build_report_product(object(), journal_request)
    ledger = app.build_report_product(object(), ledger_request)

    assert journal.content.total_debits == Decimal("160")
    assert journal.content.total_credits == Decimal("160")
    assert {line.entry_id for line in journal.content.lines} == {2}
    by_code = {item.account_code: item for item in ledger.content.accounts}
    assert by_code["1101"].opening_balance == Decimal("100")
    assert by_code["1101"].debits == Decimal("100")
    assert by_code["1101"].credits == Decimal("0")
    assert by_code["1101"].closing_balance == Decimal("200")
    assert ledger.content.total_debits == journal.content.total_debits
    assert ledger.content.total_credits == journal.content.total_credits
    assert json.loads(app.render_report_product(journal))["content"]["total_debits"] == "160"
...
def test_same_semantic_package_renders_in_explicit_binary_formats(tmp_path, monkeypatch, format_key, prefix):
    app = _wire_runtime(tmp_path, monkeypatch)
    prepared = app.prepare_financial_period_package(
        object(), from_date=date(2026, 1, 1), to_date=date(2026, 1, 31), format=format_key,
    )
...
def test_request_validation_fails_closed_for_format_filters_dimensions_and_ownership(monkeypatch):
    ...
    with pytest.raises(ValueError, match="supported"):
        app.prepare_report_request(
            object(), 1301,
            from_date=date(2026, 1, 1), to_date=date(2026, 1, 31), format="docx",
        )
    with pytest.raises(ValueError, match="supported"):
        app.prepare_report_request(
            object(), 1304,
            from_date=date(2026, 1, 1), to_date=date(2026, 1, 31), format="pdf",
        )
    ...
    foreign = ReportRequest(
        id=None, report_definition_id=1301, entity_id=2,
        from_date=date(2026, 1, 1), to_date=date(2026, 1, 31),
        as_of_date=date(2026, 1, 31), filters=(), dimensions_to_group=(), format="json",
    )
