...
def create_inkind_donation(session, donation):
    ...
    if donation.journal_line_id is not None:
        ...
        if context[0] != "active" or context[1] != "inkind_donation": raise ValueError("JournalLine lacks canonical in-kind donation provenance")
        if Decimal(line.debit) <= 0 or Decimal(line.credit) != 0 or "fixed_asset_computer_equipment" not in context[2]: raise ValueError("journal_line_id must be the recognized computer-asset debit line")
        sibling = session.exec(select(JournalLine).where(JournalLine.entry_id == entry.id, JournalLine.id != line.id)).all()
        audit = session.exec(select(__import__("aqorath.models", fromlist=["AuditEventRecord"]).AuditEventRecord).where(__import__("aqorath.models", fromlist=["AuditEventRecord"]).AuditEventRecord.event_type == "entry_posted")).all()
        matching = [event for event in audit if json.loads(event.details_json).get("entry_id") == entry.id]
        effects = [] if len(matching) != 1 else json.loads(matching[0].details_json).get("decision", {}).get("explanation", {}).get("effects", [])
        if not any(Decimal(x.credit) > 0 and Decimal(x.credit) == abs(Decimal(line.debit)) and any(effect.get("account_role") == "donation_income" and effect.get("side") == "credit" for effect in effects) for x in sibling): raise ValueError("in-kind posting must contain matching donation income")
    ...
    if donation.fixed_asset_id is not None:
        ...
        if donation.journal_line_id is not None and Decimal(asset.acquisition_cost) != abs(Decimal(session.get(JournalLine, donation.journal_line_id).debit)): raise ValueError("fixed asset value must equal recognized asset line")
