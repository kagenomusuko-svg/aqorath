...
def _stage_entity_account(session, parent_code: str, name: str):
    """Stage one governed entity Account without owning the caller transaction."""
    from sqlmodel import select
    ...
    if parent_code not in load_catalog_codes():
        raise ValueError(
    ...
    entity_account = Account(
        code=f"{parent_code}.{next_suffix:03d}",
        name=name,
...
def create_entity_account(session, parent_code: str, name: str):
    ...
    La API pública conserva la propiedad de su transacción. Composiciones de producto
    que necesiten atomicidad con otras verdades persistidas usan el helper privado de
    staging y hacen un único commit exterior.
    """
    try:
        entity_account = _stage_entity_account(session, parent_code, name)
        session.commit()
        session.refresh(entity_account)
    except Exception:
        session.rollback()
