...
from .reports_jinja import generate_pdf_from_preview, generate_pdf_for_entry
...
def report_from_preview(payload: dict, company_id: Optional[int] = None):
    """Accepts a payload with keys: template_key (str), amount (float), ctx (dict)
    Returns a generated PDF file (path) as FileResponse.
    """
    template_key = payload.get("template_key")
    amount = payload.get("amount")
    ctx = payload.get("ctx", {})
    if not template_key or amount is None:
        raise HTTPException(status_code=400, detail="template_key and amount are required")

    preview = generate_preview(template_key, float(amount), ctx)
    company = None
    if company_id:
        with get_session() as s:
...
@app.get("/reports/entry/{entry_id}")
def report_for_entry(entry_id: int, company_id: Optional[int] = None):
    """Generate a PDF for a persisted JournalEntry (reads lines from DB).
    """
    out = generate_pdf_for_entry(entry_id, company_id=company_id)
    return FileResponse(out, media_type="application/pdf", filename=out.name)